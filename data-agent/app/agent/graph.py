...
# 构建图
grap_builder= StateGraph(state_schema=DataAgentState,context_schema=DataAgentContext)
...
grap_builder.add_conditional_edges(
    "validate_sql",
    lambda state: "execute_sql" if not state["error"] else "correct_sql",
    {"correct_sql":"correct_sql","execute_sql":"execute_sql"}
)

grap_builder.add_edge("correct_sql","execute_sql")
grap_builder.add_edge("execute_sql",END)

# 编译图
graph= grap_builder.compile()
# 带引图结构
# print(grap.get_graph().draw_mermaid())
